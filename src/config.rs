...
pub struct Config {
    ...
    /// Decrypted mirror root (`<data-dir>/mirror` by default).
...
pub fn parse_args() -> anyhow::Result<Option<Config>> {
    ...
    while let Some(arg) = it.next() {
        ...
        match flag {
            ...
            "--mirror-dir" => cfg.mirror_dir = PathBuf::from(value("")),
    ...
    if cfg.mirror_dir.as_os_str().is_empty() {
        cfg.mirror_dir = cfg.data_dir.join("mirror");
    }
...
fn print_help() {
    println!(
        "weflow-server {} - headless WeChat 4.x database monitor + decrypt/extract service\n\
         ...
         \x20   --mirror-dir <DIR>         Decrypted mirror root\n\
